...
void function_xor2(vm_t *vm, processus_t *processus, int actual_pc, int *first)
{
    ...
    first_arg = get_arg(vm, processus, first_type, 8);
...
void function_xor(vm_t *vm, processus_t *processus)
{
    ...
    int first = 0, third_type = (coding_byte >> 2) & 3, third_arg = 0, sum = 0;
    processus->pc += 2; processus->carry = true;
    second_arg = get_arg(vm, processus, second_type, 8);
    third_arg = get_arg(vm, processus, third_type, 8);
    sum = second_arg + actual_pc;
    if (third_type != 1) {
        processus->carry = false; return;
    } function_xor2(vm, processus, actual_pc, &first);
    if (second_type == 1)
