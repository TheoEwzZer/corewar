...
void function_and2(vm_t *vm, processus_t *processus, int actual_pc, int *first)
{
    ...
    first_arg = get_arg(vm, processus, first_type, 6);
...
void function_and(vm_t *vm, processus_t *processus)
{
    ...
    second_arg = get_arg(vm, processus, second_type, 6);
    third_arg = get_arg(vm, processus, third_type, 6);
    sum = second_arg + actual_pc;
    if (third_type != 1) {
        processus->carry = false; return;
    } function_and2(vm, processus, actual_pc, &first);
    if (second_type == 1)
